...
| immortal_pet_v2 | 修仙人物 UI 和动画、确定性游戏规则、数值与奖励、存档、长期人格记忆、剧情事件、安全校验和离线玩法 |
| 自有服务端 | 设备鉴权、玩家账户、长期记忆、游戏上下文、模型路由、内容安全和结构化事件生成 |

原则：小智提供“耳朵、嘴巴和语音连接”，修仙人物游戏引擎决定“发生什么”和“哪些状态允许被修改”。LLM 和 MCP 均不能直接绕过游戏规则写入经验、修为、灵石、装备或存档。
...
6. 向服务端提供只读的角色状态快照，用于选择人设、语音和世界观内叙事。
...
- 服务端只能读取角色性别、境界、等级、修为、灵石、装备和当日行动等结构化状态，用于生成内容；当前阶段不通过语音直接选择行动、购买装备或突破。
- 所有游戏状态修改只由本地 UI 发起，并必须经过本地规则校验。
